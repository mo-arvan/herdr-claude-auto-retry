...
- `npm test` - 97 tests, Node's built-in runner, zero dependencies.
...
- The keystrokes recovery sends are not a blind sequence: Claude echoes the typed message into the `❯` input line, so `recovery.js` reads it back and retypes it if the first character is missing before it presses Enter (D20). Escape is a vim mode switch when Claude Code runs `"editorMode": "vim"`, so the untyped first character is a real, silent failure mode - not a theoretical one.
...
- **D20**: Recovery verifies the typed message on the echoed `❯` input line before submitting, and retypes it once if the first character was eaten. In Claude Code's vim editor mode the Escape that dismisses the `/rate-limit-options` menu (D3) also leaves the input line in NORMAL, where the message's first character runs as a vim command instead of being typed: the default message arrives as "ontinue where you left off." because `C` is change-to-end-of-line (which enters INSERT, so the remainder lands as text and the damage is exactly one character). Verified in a live pane - `esc` + text loses the character at any delay, text alone does not, so this is a mode switch and not a timing race. Repair is gated on positive evidence (the line holds the message minus its first character); an unreadable screen or any other mismatch is `unknown` and types nothing further, and Enter is always sent, so an unverifiable case is never worse than the unverified send it replaces. `verifyInput: false` restores the old blind path.
